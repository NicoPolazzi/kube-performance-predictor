...
def _make_deployment(app_label: str, cpu_raw: str) -> SimpleNamespace:
    return SimpleNamespace(
        metadata=SimpleNamespace(labels={"app": app_label}),
...
def _make_ready_deployment(generation: int = 1, replicas: int = 1) -> SimpleNamespace:
    return SimpleNamespace(
...
def _make_not_ready_deployment(generation: int = 1) -> SimpleNamespace:
    return SimpleNamespace(
        metadata=SimpleNamespace(generation=generation),
...
def test_stop_loadgenerator_patches_replicas_to_zero(mocker):
    apps_api = mocker.MagicMock()
    client = KubernetesClient(core_api=mocker.MagicMock(), apps_api=apps_api)
    client.stop_loadgenerator()
